...
def cal_target(ticker):
    df = pybithumb.get_candlestick(ticker, chart_intervals='15m')
    yesterday = df.iloc[-2]
    today = df.iloc[-1]
...
# 객체 생성
f = open("/home/chchdelm3/python/Crypto/bithumb.txt")
lines = f.readlines()
...
while True:
    ...
    coin_balance = bithumb.get_balance(ticker)  # 코인 잔고
    my_balance = bithumb.get_balance("KRW")  # 원화 잔고
    price = pybithumb.get_current_price(ticker)  # 코인 현재가
    ma = get_yesterday_ma5(ticker)  # 코인 5분 이동평균선
    limit = target * 0.08  # 손절 가격
    profit = target * 1.02
    ask_size = pybithumb.get_orderbook(ticker)[0]['total_ask_size'] # 매도 물량
    bid_size = pybithumb.get_orderbook(ticker)[0]['total_bid_size'] # 매수 물량
    ...
    try:
        ...
        if op_mode == True and price >= target and hold == False and ma < price and ask_size < bid_size:
            ...
            bithumb.buy_market_order(ticker, 10000)
            hold = True
            op_mode = False
        ...
        if op_mode == False and hold == True and limit > price:
            ...
            bithumb.sell_market_order(ticker, coin_balance)
            hold = False
            op_mode = True
